...
def on_message(client, userdata, message):
    ...
    if "type" in dson:
        ...
        if dson["type"] == "anomaly_detected":
            ...
            if "parameter" in dson and "alternatives" in dson["parameter"] and dson["parameter"]["type"] == "not_available":
                print("alternative found: " + dson["parameter"]["alternatives"][0]["alternative_object"])

                data = '{"name": "EXTERNAL_alternative", "entities": {"alternatives": "' + dson["parameter"]["alternatives"][0]["alternative_object"] + '"}}'
            ...
            data = '{"name": "EXTERNAL_success", "entities": {"success": "' + str(success) + '"}}'
            ...
            elif performedAction["action"] == "found":
                print("Found objects")
                print("target: " + performedAction["target"])
                globalFoundObjects = "objects"
                data = '{"name": "EXTERNAL_found_objects", "entities": {"found_objects": "' + performedAction["target"] + '"}}'

                topublish = makeSuccessJSON(dson)  # Python object in Json convertieren
                client.publish(topicDialStatus, topublish)

            elif performedAction["action"] == "obj_rec":
                print("Object recognition is still running")
                print("target: " + performedAction["target"])
                data = '{"name": "EXTERNAL_object_recognition", "entities": {"gegenstand": "' + performedAction["target"] + '"}}'
...
class ActionSendBring(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if tracker.get_slot('gegenstand') is not None:
            ...
            #TODO STUDIE SOLL NICHT SCHIEFGEHEN
            gegenstand = "Kekse"
...
class ActionAnomalyDetected(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        #TODO STUDIE: anomtype nicht hart setzen
        anomtype = "Bleichmittel"

        generic_utter = "Achtung! Ich habe " + anomtype + " auf dem " + place + " gefunden. " + anomtype + " ist potentiell gefährlich, soll ich es ins Regal bringen? "

        if tracker.get_slot("proactive_level") is None:
            optionInt = 3  # wenn nicht gesetzt, zufällig
        else:
            optionInt = tracker.get_slot("proactive_level")
        optionInt = 3
        if optionInt == 4:  # proaktiv, variante 3: intervention möglich, roboter räumt von allein auf
...
class ActionAnomalyReact(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        furniture = tracker.get_slot("furniture")
        anomaly = "Bleichmittel"
        furniture = "Regal3Links"
        place = "Regal3Links"
        ...
        if answertype == "deny":
           ...
           #if optionInt <= 0:
               # optionInt == 1
            dispatcher.utter_message("Okay, ich überlasse das dir. Ich bin nun mit der Besichtigung der Wohnung fertig. Du kannst mir jederzeit eine Aufgabe stellen.")

        else:
            dispatcher.utter_message("Ich bringe das " + anomaly + " zum Regal.")
            #optionInt += 1
            #if optionInt > 4:
               # optionInt == 4
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if answertype == "deny":
           ...
           # optionInt -= 2
            #if optionInt <= 0:
              #  optionInt == 1
            dispatcher.utter_message("Okay, ich bringe dir keine Alternative. Die Studie ist damit zu Ende.")
...
class ActionAlternatives(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if tracker.get_slot("gegenstand") is None or tracker.get_slot("gegenstand") == "None":
            gegenstand = "Kekse"
        else:
            gegenstand = tracker.get_slot("gegenstand")
        print("gegenstand nach if: "+ gegenstand)
        gegenstand.lower()
        pronoun_k = "kein"
        pronoun_w = "welchen"
        ...
        # proaktiv, variante 3: intervention möglich, roboter räumt von allein auf
            ...
            dispatcher.utter_message("Es gibt leider keine " + gegenstand + ". Es gibt aber noch "+  alternativeGl +
...
class ActionMoreInfo(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        global globalIntroCounter
        global globalFoundObjectCounter
        ...
        if info_type == "Greifarm" and globalIntroCounter<2:
            ...
            globalIntroCounter = globalIntroCounter + 1
            return[SlotSet("info_type", "Greifarm"), FollowupAction("utter_more_info")]
        elif info_type == "Sensoren" and globalIntroCounter<2:
        ...
        elif tracker.get_slot("info_type") == "Sensoren" and globalIntroCounter<2:
            dispatcher.utter_message("Fünf Gelenke erlauben mir meinen Arm in verschiedene Richtungen zu bewegen. Zwei Finger, die mit Kraftsensoren ausgestattet sind, erlauben es mir hierbei Gegenstände mit bis zu 3 Kilogramm sicher zu greifen.")
            globalIntroCounter = globalIntroCounter + 1
            return [SlotSet("info_type", "Greifarm")]
        elif tracker.get_slot("info_type") == "Greifarm" and globalIntroCounter<2:
            dispatcher.utter_message("In meinem Kopf befinden sich verschiedene Sensoren und eine Kamera, mit denen ich mein Umfeld, Abstände, Personen und Objekte erkennen kann. Mit meinen Lautsprechern und Mikrofonen kann ich mit Ihnen kommunizieren. Zusätzlich verfüge ich über einen Laserscanner und Ultraschallsensoren um Kollisionen zu vermeiden.")
            globalIntroCounter=globalIntroCounter+1
            return [SlotSet("info_type", "Sensoren")]
        elif globalIntroCounter == 2:
            dispatcher.utter_message("Ich erkenne, dass die Objekte im Regal in Kategorien eingeteilt sind.")
            globalFoundObjectCounter=globalFoundObjectCounter+1
            globalIntroCounter = globalIntroCounter + 1
            return [SlotSet("info_type", "Greifarm")]
        elif globalIntroCounter == 3:
            return [SlotSet("info_type", "Greifarm")]
        return
...
class ActionMemorize(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        dispatcher.utter_message("Okay, ich schaue mich um. Bitte setze dich währendessen auf das Sofa.")
        ...
        if globalLastAction is not None and success is not None:
            ...
            if globalLastAction == "anomaly_detected":
                ...
                if success == True or success == "True":
                    dispatcher.utter_message("Der gefährliche Gegenstand wurde aufgeräumt. Kann ich dir sonst noch etwas bringen?")
                else:
                    dispatcher.utter_message("Bitte helfe mir, den gefährlichen Gegenstand zu verstauen. Im Anschluss daran kann ich dir wieder assistieren.")

            elif globalLastAction == "alternatives":
                print("incoming alternatives success message")
                if success == True or success == "True":
                    dispatcher.utter_message("Die Alternative wurde erfolgreich gefunden.")
                else:
                    dispatcher.utter_message("Die Alternative konnte nicht gefunden werden.")
                ...
                if success == True or success == "True":
                    dispatcher.utter_message("Die Studie ist nun zu Ende.")
...
class FoundObjects(Action):
    def name(self):
        return "action_found_objects"

    def run(self, dispatcher, tracker, domain):

        print("action_found_objects is started")
        global globalFoundObjectCounter
        found_objects = globalFoundObjects

        if found_objects == "objects" and globalFoundObjectCounter == 0 :

            dispatcher.utter_message("Ich erkenne, dass die Objekte im Regal in Kategorien eingeteilt sind.")
            found_objects = ""
            globalFoundObjectCounter=globalFoundObjectCounter+1
        else:
            print("no useable object found")

        return

class ObjectRecognition(Action):
    def name(self):
        return "action_object_recognition"

    def run(self, dispatcher, tracker, domain):

        print("action_object_recognition is started")

        gegenstand = tracker.get_slot("gegenstand")

        if gegenstand is None or gegenstand == "None":
            gegenstand = "not_specified"
        gegenstand = gegenstand.capitalize()
        print("gegenstand: "+ gegenstand)

        if gegenstand == "Not_specified" or gegenstand == "not_specified":
            dispatcher.utter_message("Die Planung zum Greifen ist noch nicht abgeschlossen, bitte habe kurz Geduld.")
        else:
            dispatcher.utter_message("Ich habe den Gegenstand " + gegenstand + " lokalisiert, aber die Planung zum Greifen dauert noch kurz.")
